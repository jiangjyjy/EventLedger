"""Resumable OpenQA API evaluation using Monte Carlo Shapley event credit."""
...
def shapley(events, answers, n, seed):
    rng, verifier, values = random.Random(seed), OpenQAExactMatchVerifier(), [0.0] * len(events)
    for _ in range(n):
        order=list(range(len(events))); rng.shuffle(order); coalition=set(); before=0.0
        for i in order:
            coalition.add(i); after=float(verifier.verify(terminal([e for j,e in enumerate(events) if j in coalition]),answers).success)
            values[i]+=after-before; before=after
    return [v/n for v in values]

def call(cfg,prompt,seed):
    payload={"model":cfg["model"],"messages":[{"role":"user","content":prompt}],"temperature":0.2,"max_tokens":512,"seed":seed}; started=time.perf_counter(); errors=[]
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--source-run",required=True,type=Path); p.add_argument("--split-file",required=True,type=Path); p.add_argument("--openqa-data",required=True,type=Path); p.add_argument("--output-dir",required=True,type=Path); p.add_argument("--permutations",type=int,default=128); p.add_argument("--seed",type=int,default=81); p.add_argument("--model",default="glm-5.2"); a=p.parse_args()
    traces={r["task_id"]:r for r in (json.loads(x) for x in (a.source_run/"traces.jsonl").read_text().splitlines()) if r}; test=json.loads(a.split_file.read_text())["test"]; cases={c.task_id:c for c in load_nq_openqa_jsonl(a.openqa_data)}; a.output_dir.mkdir(parents=True,exist_ok=True); result=a.output_dir/"results.jsonl"; errors=a.output_dir/"errors.jsonl"; labels=a.output_dir/"shapley_labels.jsonl"; rows=[json.loads(x) for x in result.read_text().splitlines() if x.strip()] if result.exists() else []; done={r["task_id"] for r in rows}; known={r["task_id"]:r for x in labels.read_text().splitlines() if x.strip() for r in [json.loads(x)]} if labels.exists() else {}; cfg={"key":os.environ["CARVE_API_KEY"],"url":os.environ.get("CARVE_BASE_URLS","https://api.openai.com/v1").rstrip("/").removesuffix("/v1"),"model":a.model}; verifier=OpenQAExactMatchVerifier()
    for i,tid in enumerate(test):
        if tid in done: continue
        trace=traces[tid]; events=trace["events"]; label=known.get(tid)
        if label is None:
            label={"task_id":tid,"permutations":a.permutations,"values":shapley(events,cases[tid].answers,a.permutations,a.seed+i)}
            with labels.open("a") as h: h.write(json.dumps(label)+"\n"); h.flush(); os.fsync(h.fileno())
        kept=[e for e,v in zip(events,label["values"]) if v>0 and e.get("type") not in {"aggregate","stop"} and not re.search(r"final\s+answer\s*:",str(e.get("content","")),re.I)]; prompt="Return only the shortest exact answer span, with no explanation. Do not copy an old final answer.\n\nQuestion:\n"+cases[tid].question+"\n\nShapley-selected evidence:\n"+"\n".join(str(e.get("content","")) for e in kept)
        try:
            started=time.perf_counter(); answer,tel=call(cfg,prompt,i); score=verifier.verify(answer,cases[tid].answers); row={"task_id":tid,"success":bool(score.success),"answer":answer,"api_calls":1,"events_kept":len(kept),"elapsed_seconds":time.perf_counter()-started,"telemetry":tel}

...
def _episode_reward(trace: Trace, actions: list[int], event_scores: dict[str, float], verifier=None, reference=None) -> tuple[float, dict[str, Any]]:
    ...
    *,
    ...
    for trace in traces:
        ...
        if verifier is None:
            ...
            result = verifier.verify(answer, (references or {})[trace.task_id])
            verifier_score = float(result.score) if getattr(result, "score", None) is not None else None
            oracle_score = None
            success = bool(result.success)
        controlled.append(
        ...
        _, record = _episode_reward(trace, actions, event_scores, verifier, (references or {}).get(trace.task_id))
...
def main() -> None:
    ...
    parser.add_argument("--credit-source", choices=("carve_s", "uniform"), default="carve_s")
    ...
    split = _read_split(Path(args.split_file) if args.split_file else student_run / "split.json")
    ...
    if args.dataset.lower() == "spider":
        ...
        for trace in train_traces + eval_traces:
            ...
            refs[trace.task_id] = case
    elif args.dataset.lower() == "openqa":
        if args.openqa_data is None:
            raise ValueError("--openqa-data is required for OpenQA")
        for line in args.openqa_data.read_text(encoding="utf-8").splitlines():
            row = json.loads(line)
            refs[str(row["task_id"])] = tuple(str(x) for x in (row.get("reference") or {}).get("answers", []))

    scored_traces = train_traces + eval_traces
    if args.credit_source == "uniform":
        student_scores = {
            f"{trace.trace_id}::{event.event_id}": 1.0 / max(1, len(trace.events))
    ...
    for update_index, trace in enumerate(train_traces):
        ...
        for group_index in range(args.group_size):
            ...
            verifier = SpiderVerifier() if args.dataset.lower() == "spider" else OpenQAExactMatchVerifier() if args.dataset.lower() == "openqa" else None
            reward, details = _episode_reward(trace, actions, student_scores, verifier, refs.get(trace.task_id))
    ...
    if args.dataset.lower() in {"spider", "openqa"}:
        student_control_summary = {"controlled_traces": policy_summary["traces"], "controlled_success_rate": policy_summary["success_rate"], "controlled_mean_tokens": policy_summary["mean_tokens"], "mean_removed_events": policy_summary["mean_removed_events"]}
    else:
        student_eval_scores = filter_event_scores_for_traces(eval_traces, student_scores)
        student_controlled, student_control_summary = evaluate_controlled_traces(eval_traces, student_eval_scores)
    ...
    _write_json(
        output_dir / "summary.json",
        {
            ...
            "credit_source": args.credit_source,
